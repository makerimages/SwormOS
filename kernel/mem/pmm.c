...
void pmm_init(size_t memSize, uint32_t bitmap) {
    ...
    kprintf("\tPMM initialized with %d blocks of memory. %d currently in use Map at 0x%x\n",max,used,manager_map);
...
void deinit_bitmap() {
    deinit_region(manager_map,sizeof(manager_map));
}
